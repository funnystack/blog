...
秒杀是一个读多写少的场景，使用redis做缓存再合适不过；在上面提到的分层限流中，大多数都是通过使用缓存来实现，缓存的高可用性将影响到整个系统的高可用性。秒杀场景下，单个 Redis 实例在内存和CPU利用率上往往会显得捉襟见肘，这时候需要高可用redis集群来保证系统的可用性和扩展性。

![1596792169590](images/seckill/1596792169590.png)
...
在我们制定完系统的性能指标参考值后，借助之家云平台提供的压测工具AutoTPS，我们在有限的资源里实现了对整个系统的端到端全链路压测。一次次的全链路压测参考指标，帮助我们了解了性能的基准，并以此做了代码设计层面、容器层面、JVM 层面、MySQL 数据库层面、缓存集群层面的种种优化，极大的提升了系统的可用性。具体做法限于篇幅不在此赘述，有兴趣的读者欢迎交流。
...
订单收单流程是属于重保系统，为了保证秒杀系统挂了，订单收单也能够正常进行，需要将收单的秒杀路径与正常下单路径隔离部署，保证互不影响。
