...
由于MySQL存储数据的特点，同一数据在数据库里肯定是一行存储（MySQL），因此会有大量线程来竞争InnoDB行锁，而并发度越高时等待线程会越多，TPS（Transaction Per Second，即每秒处理的消息数）会下降，响应时间（RT）会上升，数据库的吞吐量就会严重受影响。所以推荐使用方法1。如果change_log表很大，写入耗时性能较差，可以考虑定期对历史数据进行归档。
...
通过对比结算页的url的访问数量和下单url的访问数量，发现大多数黑产都是直接刷的下单url，所以我们加了

从结算页到用户下单必须要有大于2s的操作。
...
通过查询nginx的历史流量，我们发现抢购的过程中，大量请求都是非真人请求，为了能减少用户的秒杀路径，所以减少了排队等待时间。为了然后真正的用户下单也是煞费苦心。
